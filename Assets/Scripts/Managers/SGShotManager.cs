...
{
    private List<SGShotCtrl> m_shotList = new List<SGShotCtrl>(2048);           //총알을 2048개 까지 관리하는 list
    private HashSet<SGShotCtrl> m_shotHashSet = new HashSet<SGShotCtrl>();      //heshset 값으로 샷 컨트롤 리스트 관리
    public int activeShotCount { get { return m_shotList.Count; } }             //activeshot 접근 할 경우 갯수 리턴

    public void UpdateShots(float deltaTime)                                    //총알 쏜 것 Update 지속적으로 시켜주고 지워야 할 것들 지움
    {
